...
export default function DiaryUpdateScreen() {
    ...
    useEffect(() => {
        if (!id) return;
        diaryService.getOne(id)
            .then(entry => {
                setTitle(entry.title);
                setDiaryEntry(entry.content ?? '');
                setSelectedEmote(Array.isArray(entry.emotes) ? (entry.emotes[0] ?? 'neutral') : 'neutral');
            })
            .catch(() => toastService.error('Não foi possível carregar o registro.'))
    ...
    const handleUpdate = async () => {
        ...
        try {
            setSaving(true);
            await diaryService.update(id, { title, content: diaryEntry, emotes: [selectedEmote] });
            toastService.success('Registro atualizado!');
            router.back();
        } catch (e: any) {
            const msg = e?.response?.data?.message || 'Não foi possível atualizar o registro.';
    ...
    const isFormValid = title.trim().length > 0 && diaryEntry.trim().length > 0;
                    ...
                    <View style={localStyles.footerContainer}>
                        <TouchableOpacity
                            style={[
                                ...
                                { backgroundColor: (isFormValid && !saving) ? Colors.purplePrimary : '#E0E0E0' },
                            ]}
                        ...
                        >
                            <View style={localStyles.saveButtonContent}>
                                <Text style={localStyles.saveButtonText}>
                                    {saving ? 'Atualizando...' : isFormValid ? 'Atualizar Registro' : 'Campos obrigatórios'}
                                </Text>
